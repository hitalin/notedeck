...
builtin は **69 個** (v0.24.0 時点、`src/capabilities/builtins/` 配下の unique id)。subject 別にグループ化:
...
| **アカウント** | `account.current`, `account.list` | 自アカウント / 全アカウント情報 |
| **メタ** | `meta.permissions`, `meta.activeSkills`, `meta.persona`, `meta.config` | 自分自身 (AI の権限・skill・persona・config) を内省 |
| **カラム** | `column.list`, `column.active`, `column.add`, `column.remove`, `column.focusedNote` | デッキカラムの操作 + フォーカスノート取得 |
| **ノート (read)** | `notes.timeline`, `notes.user`, `notes.search`, `notes.show`, `notes.children` | TL / ユーザー別 / 検索 / 単発取得 / 返信ツリー |
| **ノート (write)** | `notes.create`, `notes.post`, `notes.react` | 投稿・リアクション (`notes.write` / `notes.react` permission) |
| **通知** | `notifications.list` | 通知一覧 |
| **ドライブ** | `drive.list` | ファイル一覧 |
| **ユーザー** | `user.lookup` | ユーザー検索 |
| **メモ** | `memos.list`, `memos.search`, `memos.backlinks`, `memos.create`, `memos.update`, `memos.delete` | AI 永続記憶用ローカルメモ CRUD (#492 #494) |
| **テーマ** | `theme.list`, `theme.read`, `theme.apply`, `theme.create`, `theme.update`, `theme.history`, `theme.revert` | per-account テーマ CRUD + 編集履歴。`theme.create` は `installedFor` 自動紐付け、`theme.apply` はモード自動切替 |
| **スキル** | `skills.list`, `skills.read`, `skills.append`, `skills.replaceSection`, `skills.toggle`, `skills.history`, `skills.revert` | skill 自己編集 (write 系は `aiTool:false` ガード — 4.4 参照) |
| **ウィジェット** | `widgets.list`, `widgets.read`, `widgets.create`, `widgets.update`, `widgets.setAutoRun`, `widgets.delete`, `widgets.history`, `widgets.revert` | AiScript widget の AI 編集 |
| **プラグイン** | `plugins.list`, `plugins.read`, `plugins.create`, `plugins.update`, `plugins.setActive`, `plugins.delete`, `plugins.history`, `plugins.revert` | AiScript plugin の AI 編集 |
| **AI 内省** | `ai.chat`, `ai.sessions.list`, `ai.sessions.read`, `ai.sessions.search` | プラグインから本体 AI を呼ぶ / 過去セッション参照 |
...
- 自己編集系は `safe` でも許可だが、capability 側に `aiTool: false` ガードがあるため AI からは見えない (§5.2)
...
### 5.2 `aiTool:false` ガード (自己改変系)

skill / widget / plugin / theme の **書き込み系 capability** (`skills.append` / `skills.replaceSection` / `widgets.create|update|delete` / `plugins.create|update|delete|setActive` / `theme.create|update|revert`) は `aiTool: false` 属性で **デフォルトでは tool calling に露出しない**。AI に編集を許可するには capability ごとに明示的に有効化する必要がある。これは「AI が自己拡張する IDE」の安全弁。
...
| 自己改変系 capability (skill/widget/plugin/theme write) | `aiTool:false` で隠蔽 | 個別に明示有効化が必要 |
