...
| **P5-A** Note Store 再設計 | 2-4 週間 | 持続スクロール時の再描画・GC を大幅削減 | DevTools Performance → streaming 中の scripting / forced re-render |
| **P5-B** Rust 側 stream batch / appview 化 | 3-6 週間 | 高流量サーバーで IPC / dispatch / sort コストを削減 | misskey.io 級 TL で `stream-event` 発火回数と CPU を比較 |
...
- [Phase 2: プロファイリング後に判断](#phase-2-プロファイリング後に判断)
  ...
  - [P5-A: Note Store の Entity/Timeline 分離](#p5-a-note-store-の-entitytimeline-分離)
  - [P5-B: Rust 側 stream batch / appview 化](#p5-b-rust-側-stream-batch--appview-化)
...
| 画像最適化 | プリフェッチ | `src/composables/useImagePrefetch.ts` |
| OGP キャッシュ | LRU (128) | `src/composables/useOgpPreview.ts` |
| コード分割 | 142 `defineAsyncComponent` + `manualChunks` | `vite.config.ts` |
| Multi-tier cache | メモリ LRU → ファイルディスクキャッシュ → ネットワーク | `src-tauri/src/image_cache.rs` |
...
| カラム遅延表示 | IntersectionObserver + paused 制御 | `src/composables/useColumnVisibility.ts` |
...
| Query Subscription state machine | `Live` ↔ `Warm` ↔ `Suspended` を Rust `QueryRuntime` が自動遷移、ColumnMountRegistry の visibility に連動。各 `Deck*Column` を query subscriber に統一 | `src/composables/useQuerySubscription.ts`, `src-tauri/src/query_runtime.rs` |
| queryDelta 16ms debounce | stream batch を Rust 側で時間窓まとめて 1 回 emit (`a35f7321` "always run note capture") | `src-tauri/src/query_runtime.rs` |
...
| Vapor Mode 準備完了 | ブロッカーゼロ。errorHandler も onErrorCaptured に移行済み | `src/composables/useVaporTransition.ts` |
| Notes/Chat Cache eviction policy | preset (省メモリ / バランス / 高パフォーマンス) + custom controls、ユーザー設定で eviction を制御 (`47c6ab58`, `3c5e4cdd`) | `src/components/window/CacheEditorContent.vue` |
| Chat メッセージキャッシュ (#460) | ローカル DB cache + ログアウト後閲覧 + 起動時 hydrate + WS reactions UI 反映 + thread prefetch (全件 pagination) | `src/components/deck/DeckChatColumn.vue` + Rust `chat_messages_cache` |
| Memo link expand (AI context) | `memo:<id>` markdown link で本文展開 + `referencedBy` 添付 (#494) | `src/composables/useAiSystemContext.ts` |
...
- `rust_ogp_cache_max: 256` — OGP キャッシュエントリ数
- `circuit_breaker_threshold: 5` — 連続失敗で遮断
- `memory_cache_max_total: 32MB` — メモリキャッシュ上限
...
> **状態: 別アプローチで達成済み (v0.13 系)** — 個別の `stream_unsubscribe` を JS から呼ぶ
> 方式ではなく、`useQuerySubscription` + Rust 側 `QueryRuntime` の **state machine
> (`Live` ↔ `Warm` ↔ `Suspended`)** で自動管理されている。`ColumnMountRegistry`
> の visibility 連動で画面外カラムは Suspended に遷移し、JSON parse → emit のコストが消える。
> 再表示時は `Suspended → Live` 遷移と同時に snapshot reload + delta merge が走る。
> 関連 commit: `82e82bee` / `1e758f53` / `b1d38960` / `83700021` / `a7560f14` / `907f606f` /
> 各 `Deck*Column` の query subscriber 化 (`6ba4afac` / `34dfa6d6` / `b4f5ea14` / `5e63d826` /
> `538c06c9` / `d62f7b17` / `5a6bd6f1`)。
...
#### 現状の問題 (当時の課題、現在は QueryRuntime で解消)
...
> **状態: 部分的に実装済み** — toast 通知は実装 (overflow 通知行を参照)。
> サブスクリプション一時停止は `streamAdapter.pause()` ではなく、
> 上記 QueryRuntime の `Suspended` 遷移で自動的に行われる。
> 当初案の手動 pause API は不要となった。
...
### P5-A: Note Store の Entity/Timeline 分離

**目的**: Misskey らしいリッチなノート表示を維持したまま、持続的なヌルヌルさを優先する。

#### 背景

現行の `useNoteList()` は column ごとに `orderedIds` を持ちつつ、描画時に
`noteStore.resolve(orderedIds)` で `NormalizedNote[]` を都度再構築する。
`notes` store も `triggerRef(noteMap)` により map 全体を invalidate するため、
1 件のリアクション更新や poll 更新でも列単位で computed が再評価されやすい。

#### 現状の問題

- `orderedIds -> resolve() -> notes[]` の変換が streaming 中に何度も走る
- `noteMap` の全体 invalidation が、更新対象の 1 行を超えて波及しやすい
- 仮想スクロールで DOM 数は抑えられているが、Vue 側の再評価粒度はまだ粗い

#### 改善案

`notes` を 2 層に分離する。

- **Timeline Store**: column / query ごとの `ids: shallowRef<string[]>`
- **Entity Store**: `noteId -> shallowRef<NormalizedNote>`

描画側は `NormalizedNote[]` を丸ごと受け取らず、`noteId[]` を仮想リストへ渡す。
各行コンポーネントが `noteId` から自分の entity ref だけ購読する。

```
stream event
  → entityStore.update(noteId, patch)
  → timelineStore.prepend/append(ids)
  → visible row only re-render
```

#### 実装方針

1. `useNoteList()` の `resolve()` 依存を廃止し、`ids` ベースへ移行
2. `MkNote` まわりを `note object` 入力から `noteId` 入力へ段階移行
3. `notes` store を `Map<string, shallowRef<NormalizedNote>>` ベースへ変更
4. `applyUpdate()` を「全体 trigger」ではなく「対象 entity のみ更新」に寄せる

#### 期待効果

- streaming 中の scripting time と GC 圧の低下
- reaction / poll / delete のような局所更新で列全体が揺れにくくなる
- `Rust 側 batch / appview` を後段で導入してもフロントで取りこぼしにくくなる

#### 優先順位

Misskey 本家に近いリッチ表示を維持する前提では、**軽量表示モードより優先**。
`Rust 側 batch / appview` を入れる前の受け皿としても先に実施する。

---

### P5-B: Rust 側 stream batch / appview 化

**目的**: 高流量サーバーでも `stream-event` の細粒度 dispatch を減らし、IPC と JS 側の負荷を下げる。

#### 背景

現行の Rust 側は stream event を 1 件ずつ `app.emit("stream-event")` し、
フロントが subscription ごとに dispatch している。
通常サーバーでは十分だが、グローバル TL や大規模サーバーでは
IPC 回数・listener dispatch・front 側 merge の固定費が積み上がる。

#### 現状の問題

- イベントが多いほど Tauri event の発火回数が直線的に増える
- JS 側で「受信 → filter → dispatch → insertIntoSorted」を小刻みに繰り返す
- hidden / non-live column を絞っても、account 単位の高流量では境界コストが残る
...
- Rust 側で 16-33ms 程度の時間窓ごとにイベントをバッファ
- `notes[]`, `updates[]`, `notifications[]` をまとめて 1 回 emit

**Step 2: lightweight appview / materializer**

- Rust/SQLite 側で timeline ごとの最新 ID 列と差分を保持
- フロントは「生イベント列」ではなく「整形済み snapshot / delta」を受け取る

```
WebSocket
  → Rust batch buffer
  → timeline materializer (dedup / sort / filter)
  → emit(batch or delta)
  → Vue timeline store
```
...
1. まず `stream-event` の batch 化だけ導入する
2. その後、特に高流量な timeline 種別だけ materializer を試験導入する
3. 最後に SQLite-backed appview に寄せるかを再評価する

#### 期待効果

- 高流量サーバーでの CPU 使用率低下
- JS listener dispatch 回数の削減
- frame 落ちしやすい burst 時の安定化

#### 優先順位

`P5-A Note Store 再設計` の後に行う。フロントの再描画粒度が粗いままだと、
Rust 側だけ batching しても効果が頭打ちになるため。
...
`parseMfm()` はメインスレッドで同期実行。再帰パーサー + 15 正規表現パターン + 6 ブロックパターン。
...
~~楽観的更新は実装済みだが、オフライン時の操作キューイングは未実装。~~

**廃止理由**: Misskey のリアルタイム性を壊すため実装しない。
オフラインモードでは書き込み操作をブロックする現行動作を維持する。
