...
#[tauri::command]
pub fn set_window_vibrancy(window: tauri::WebviewWindow, effect: String) -> Result<()> {
    use tauri::window::{Effect, EffectsBuilder};

    let effects = match effect.as_str() {
        // macOS
        "vibrancy" => EffectsBuilder::new()
            .effect(Effect::UnderWindowBackground)
            .build(),
        "sidebar" => EffectsBuilder::new().effect(Effect::Sidebar).build(),
        // Windows 11
        "mica" => EffectsBuilder::new().effect(Effect::Mica).build(),
        "mica-dark" => EffectsBuilder::new().effect(Effect::MicaDark).build(),
        "mica-light" => EffectsBuilder::new().effect(Effect::MicaLight).build(),
        ...
        "auto" => {
            ...
            #[cfg(target_os = "linux")]
            {
                // Linux has no native vibrancy; just clear effects
                return window.set_effects(None).map_err(|e| {
                    NoteDeckError::InvalidInput(format!("Failed to clear effects: {e}"))
                });
            }
        }
        _ => {
    ...
    window
        .set_effects(effects)
        .map_err(|e| NoteDeckError::InvalidInput(format!("Failed to set window effects: {e}")))
}

#[tauri::command]
pub fn clear_window_vibrancy(window: tauri::WebviewWindow) -> Result<()> {
    window
        .set_effects(None)
        .map_err(|e| NoteDeckError::InvalidInput(format!("Failed to clear window effects: {e}")))
}
