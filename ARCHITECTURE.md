...
#### C-2. DB マイグレーションの限界 [重要度: 中]

現在のインクリメンタルマイグレーションは `ALTER TABLE ADD COLUMN` のみ対応。

**問題**: テーブル構造の大幅変更（カラム削除、型変更、テーブル分割等）に対応できない。機能追加に伴いスキーマが複雑化すると破綻する可能性がある。

**改善案**: バージョンテーブル (`schema_version`) を導入し、マイグレーション関数を番号付きで順次実行する方式へ移行。
...
### D-3. Sidecar + ローカル AI 統合 ★★★

**概要**: Tauri の sidecar 機能で Ollama やローカル LLM を起動し、NoteDeck 内で AI 機能を提供。

活用例:
- タイムラインの要約（「今日の misskey.io で何があった？」）
- 投稿の翻訳・校正（ノート作成時に AI が提案）
- コンテンツフィルタリング（不快なノートを自動非表示）
- リアクション提案（ノート内容に基づく絵文字推薦）

**UX効果**: クラウドに送信せずプライバシーを保ちながら AI 機能を利用。

**実装コスト**: 中。Ollama HTTP API との連携は単純だが、プロンプト設計とモデル管理が必要。
...
**UX効果**: macOS Vibrancy / Windows 11 Mica のようなネイティブ感のある美しい UI。

**実装コスト**: 低。Tauri API の設定変更のみ。ただし CSS 側のテーマ調整が必要。
...
活用例:
- スクリーンショット → Ctrl+V で即座に投稿フォームに添付
...
### D-6. マルチプロセス・サンドボックス化プラグイン ★★

**概要**: AiScript プラグインを独立したサンドボックスプロセスとして隔離実行。
...
**UX効果**: 悪意のあるプラグインからのデータ漏洩を防止。安心してサードパーティプラグインを利用可能。

**実装コスト**: 高。プラグイン API の設計・権限モデルの策定が必要。
...
**UX効果**: クラウド不要のローカルデバイス間同期。

**実装コスト**: 高。デバイス発見・認証・競合解決の設計が必要。
...
OS統合               │ A-7 トレイ/ショートカット │ C-5 URI スキーム │ D-2 ディープリンク
                     │                     │                    │ D-4 ウィンドウ透過
                     │                     │                    │ D-5 クリップボード
プラグイン           │                     │ C-7 AiScript       │ D-6 サンドボックス
AI連携               │                     │                    │ D-3 Sidecar LLM
デバイス間同期       │                     │                    │ D-7 P2P同期
```
...
| 順位 | 項目 | 理由 |
|------|------|------|
| 1 | C-1 HTTP サーバー統合 | 技術的負債の解消。API 一貫性の確保 |
| 2 | C-3 テスト追加 | リグレッション防止の基盤 |
| 3 | D-1 オフラインファースト | 既存 SQLite 基盤の拡張で実現可能。モバイル UX を大幅改善 |
| 4 | D-2 ディープリンク | 低コストで Web↔アプリ連携を実現 |
| 5 | D-3 Sidecar ローカル AI | 他の Misskey クライアントにない差別化要素 |
