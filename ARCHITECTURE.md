...
WebSocket 接続は維持したまま、subscription 単位で「受信→正規化→DB cache→emit」を停止できる。
off-screen / `maxLiveColumns` 予算外のカラムでもストリーム断はせず、購読の active フラグで送出だけを止める。
JS 側 `MisskeyStream` は `liveCount` と warm timer（既定 8s）を使って `live → warm → suspended` の状態遷移を自動で行う。
...
| リアルタイム通信 | A-4 マルチ配信ブリッジ / A-4b suspend/resume |
