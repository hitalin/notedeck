...
**概要**: SQLite キャッシュを活用し、オフライン時もキャッシュ済みタイムラインを閲覧可能にする。読み取り専用のオフライン対応。
...
graph LR
    ...
    subgraph Offline["オフライン"]
        ...
        Write["投稿・リアクション"] -->|エラー表示| Block["オフラインでは実行不可"]
    end
    subgraph Reconnect["復帰時"]
        Gap["キャッシュの続きから新着フェッチ"] --> Sync["UI 同期"]
    end
```

**方針**: 書き込みキューイングは行わない。Misskey はリアルタイム性が重要な SNS であり、オフライン時に蓄積したリアクションや投稿を後から送信しても文脈が失われる。書き込み操作はオンライン時のみ実行し、オフラインなら素直にエラーを返す。

**現状との差分**: `api_get_cached_timeline` でキャッシュ読み出しは可能だが、**オフライン検出→キャッシュ自動切替**と**復帰時のギャップフェッチ**がない。

**UX効果**: 電車の中や Wi-Fi 不安定な場所でもタイムラインを快適に閲覧。接続復帰時にキャッシュの続きからシームレスに新着を表示。

**実装コスト**: 低〜中。既存の SQLite キャッシュ基盤をそのまま活用。
...
| - | D-2 ディープリンク | 保留。非公式クライアントが Misskey URL を横取りするのは不自然 |
