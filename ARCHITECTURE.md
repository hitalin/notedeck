...
- [第2部: 残課題](#第2部-残課題)
- [第3部: レンダリングパイプライン](#第3部-レンダリングパイプライン--ゲームエンジン発想の-ui-最適化)
- [採用状況マトリクス](#採用状況マトリクス)
...
## 第3部: レンダリングパイプライン — ゲームエンジン発想の UI 最適化

ゲームエンジンの「16ms フレーム予算で 60fps」という規律を、DOM ベースの UI に適用する。DOM が無料で提供するテキスト選択・アクセシビリティ・i18n を活かしつつ、描画パイプラインの無駄を削る。
...
| ゲームエンジン概念 | NoteDeck 実装 | 場所 |
|---|---|---|
| **Frame Budget** | RAF バッチング（WS 受信を蓄積→次フレームで一括フラッシュ） | `useStreamingBatch.ts`, `notes.ts` |
| **Object Pooling** | `@tanstack/vue-virtual` DOM リサイクル（実 DOM 30-50件、データ最大200件） | `NoteScroller.vue` |
| **LOD** | `content-visibility: auto` + `contain: layout style paint` | `NoteScroller.vue` |
| **GPU Offload** | `translate`（位置）と `transform`（アニメーション）を分離、compositor-only | `NoteScroller.vue` |
| **Dirty Flag** | RAF-batched `triggerRef`（1フレーム1リアクティブ更新） | `notes.ts` |
| **Prefetch Pipeline** | 2段階ゾーン（near viewport + extended range） | `NoteScroller.vue`, `useImagePrefetch.ts` |
| **Tuning Presets** | low / balanced / high プリセット | `performance.ts` |

**CSS レンダリング規約:**

- **Compositor-only アニメーション**: `transform`, `opacity`, `translate`, `scale`, `rotate` のみ。`width`/`height`/`top`/`left` 等は禁止
- **Layout Thrashing 回避**: DOM 読み取り（`offsetHeight` 等）と書き込みを交互に行わない
- **CSS Containment**: スクロール内アイテムに `contain: layout style paint` + `content-visibility: auto`
...
| composable | 役割 | 場所 |
|---|---|---|
| `useFrameScheduler` | 優先度付き RAF/idle スケジューラ（high → normal → idle の3段階） | `useFrameScheduler.ts` |
| `useAdaptiveQuality` | デバイス性能検出 → performance プリセット推奨（CPU コア / RAM / フレームドロップ率） | `useAdaptiveQuality.ts` |

既存の個別 RAF パターン（`useStreamingBatch.ts`, `notes.ts` 等）は動作しており、新規コードから段階的に `useFrameScheduler` に移行可能。

**将来の最適化候補:**

| ID | 内容 | 状態 |
|---|---|---|
| P-1 | OffscreenCanvas Worker（絵文字デコード・MFM エフェクトのメインスレッド分離） | 検討中 |
...
| レンダリングパイプライン | 第3部: Gaming CSS（RAF バッチ / 仮想スクロール / GPU Offload / CSS Containment / useFrameScheduler / useAdaptiveQuality） |
