...
|---|---|---|
| **Frame Budget** | `useFrameScheduler` 経由の優先度付き RAF バッチング（WS 受信を蓄積→次フレームで一括フラッシュ） | `useStreamingBatch.ts`, `notes.ts` |
...
| **Dirty Flag** | `useFrameScheduler('normal')` で `triggerRef` をバッチ（1フレーム1リアクティブ更新） | `notes.ts` |
| **Prefetch Pipeline** | 2段階ゾーン（near viewport + extended range） | `NoteScroller.vue`, `useImagePrefetch.ts` |
| **Tuning Presets** | low / balanced / high プリセット + `useAdaptiveQuality` による自動推奨 | `performance.ts` |
...
| composable | 役割 | 統合先 |
|---|---|---|
| `useFrameScheduler` | 優先度付き RAF/idle スケジューラ（high → normal → idle の3段階） | `useStreamingBatch.ts`, `notes.ts` |
| `useAdaptiveQuality` | デバイス性能検出 → performance プリセット推奨（CPU コア / RAM / フレームドロップ率） | `performance.ts`（init 時に自動検出、設定 UI に「おすすめ」バッジ表示） |
