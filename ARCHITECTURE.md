...
- [第2部: 残課題](#第2部-残課題)
- [第3部: レンダリングパイプライン — Gaming CSS](#第3部-レンダリングパイプライン--gaming-css)
- [採用状況マトリクス](#採用状況マトリクス)
...
## 第3部: レンダリングパイプライン — Gaming CSS

ゲームエンジンの「16ms フレーム予算で 60fps」という規律を、DOM ベースの UI に適用する。DOM が無料で提供するテキスト選択・アクセシビリティ・i18n を活かしつつ、描画パイプラインの無駄を削る。

8 レイヤー構成（Input Manager / Frame Engine / Animation System / Visibility Engine / Asset Pipeline / Compositor Layer Manager / Performance Telemetry / Worker Pool）で、統一 RAF ループ・フレーム予算管理・自動品質調整を実現する。

詳細設計・CSS 規約・移行計画・実装状況は **[DEVELOPMENT.md — Gaming CSS v2](DEVELOPMENT.md#gaming-css-v2--レンダリングエンジンアーキテクチャ)** を参照。
...
| レンダリングパイプライン | 第3部: Gaming CSS → [DEVELOPMENT.md](DEVELOPMENT.md#gaming-css-v2--レンダリングエンジンアーキテクチャ) に詳細 |
