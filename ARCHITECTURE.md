...
- **同時 live 数の budget**（`maxLiveColumns`）: マウントされたカラムは全て live になる
- **Hydration State Machine**: mounted/unmounted の 2 状態のみ。shell 段階での snapshot 表示がない
- **shell → snapshot → live の段階的表示**: 現行は mount 即 connect
...
#### Phase 1. Adapter Cache 導入

`useInitAdapter.ts` に accountId 単位の adapter cache を追加する。

- `initAdapterFor()` で既存 adapter があればそれを返す（`useMultiAccountAdapters` と同じパターン）
- MisskeyApi / MisskeyStream インスタンス数を account 数に制限
- adapter の寿命は column unmount で終わらない（account 切替・ログアウト時に破棄）

**不要な作業**（既に実装済み）:
- ~~`HostSessionStore`~~ → `serversStore` + `emojisStore` が機能中
- ~~`getServerInfo()` の in-flight dedup~~ → `serversStore` に実装済み

この段階では UI 構造は変えず、JS 側の adapter 重複のみ解消する。
...
#### A-1. JS 側 stream dispatcher の単一化

現行は Rust 側の WebSocket 接続は account 単位で dedup されているが、JS 側では `MisskeyStream` インスタンスごとに `listen('stream-event')` を登録している。

このため、同一 account に複数 column がぶら下がると、Rust 側の接続数は増えなくても、JS 側の event listener / dispatch コストは column 数に応じて増える。

**改善案**:

- JS 側に account 単位、またはアプリ全体で 1 つの `stream-event` listener を持つ
- `subscriptionId` / `accountId` による routing のみを共通 dispatcher が担当する
- column は dispatcher に observer として参加し、独自 listener は持たない

**期待効果**:

- 起動時: **中〜大**
- 多 column 常時利用時の CPU: **中〜大**
- adapter singleton 化との相乗効果: **大**

**位置づけ**:

- 実質的には Phase 1 の一部として扱うのが自然
- `adapter cache` だけで止めず、`stream listener fan-out` までまとめて潰した方が設計としてきれい

#### A-2. Initial Hydration の batch 化

現行の `useNoteColumn()` は、各 column が個別に cache restore, adapter 初期化, streaming 準備, API fetch を進める。

そのため active / neighbor だけに絞っても、初回表示では「複数 column が似た処理を別々に走らせる」構造が残る。

**改善案**:

- Rust / IPC に「visible columns の初期 hydrate をまとめて返す」入口を追加する
- 返却内容は `snapshot IDs`, `latest note metadata`, `server badge 用情報`, `diff fetch に必要な cursor` など、first paint に必要な最小集合に絞る
- column ごとの `connect()` は段階的 upgrade に使い、初回表示の責務を batch hydration に寄せる

**期待効果**:

- splash 後の first contentful paint: **中〜大**
- cold start 時の IPC 往復削減: **中**
- 実装コスト: **高**

**位置づけ**:

- Phase 1-2 の後でもよいが、もし起動体感がまだ重いなら優先的に検討する価値がある
- `maxLiveColumns` と競合しない。むしろ相性が良い

### 今は優先度を上げなくてよい施策

以下は改善余地自体はあるが、現時点では「劇的改善の本命」ではない。

#### N-1. Read Model の先行導入

Read Model / fine-grained invalidation は重要だが、主に steady-state の反応性最適化である。

- 起動 fan-out を止める主施策ではない
- Session / Viewport 改善の前に入れると、効果の切り分けが難しい

そのため、位置づけは引き続き **Phase 3** でよい。

#### N-2. `MkNote` / `MkMfm` の全面軽量化を先にやること
...
1. adapter singleton 化 + JS stream dispatcher 集約
2. `maxLiveColumns` + shell / snapshot / live の hydration state machine
3. initial hydration の batch 化
4. Read Model / fine-grained invalidation
5. lightweight first paint の拡張

この順序であれば、起動体感に効く施策から順に導入でき、かつ各段階で効果測定と rollback がしやすい。
...
| カラム数が増えても同時 live 数が一定 | Phase 2 | **未達成**（mount = live） |
| off-screen column が CPU と stream listener をほぼ消費しない | 既存 | **部分達成**（pause + unsub 済み、budget なし） |
| splash 後 500ms 以内に shell または snapshot が見える | Phase 2 | **部分達成**（skeleton あり、snapshot 表示なし） |
| note update が他カラムの visible list 再計算を起こさない | Phase 3 | **未達成**（計測後に判断） |
...
| 同時 live 数制限 | なし（mount = live） | **未実装** | `useColumnVisibility` に `maxLiveColumns` 追加 |
| 可視性制御 | `useColumnVisibility` + `eagerMountRadius` | **実装済み** | budget 概念を追加 |
| off-screen pause | `useNoteColumn` で pause + unsubscribe | **実装済み** | 変更不要 |
| snapshot 復元 | `useSnapshotStore`（ID-only, TTL） | **実装済み** | shell 段階での表示に拡張 |
| note invalidation | `noteStore`（RAF batch + skipTrigger） | **実装済み** | Phase 3 で計測後に判断 |
