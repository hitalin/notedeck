...
> **Status**: Phase A (notecli 側 + frontend インフラ) 実装済 (#460)、Phase B (UI 統合 + offline-first hydrate + gap reconcile) 未着手。`feat/chat-cache-460` ブランチ参照。
...
| FTS opt-in | 「note は検索したい / チャットは検索したくない」というユーザー選好を分離したい |
...
| **FTS5** | v1 では作らない (Phase C で opt-in 検討) |
| **`text` 列** | 持たない (Phase C の FTS 導入時に generated column か trigger で乗せる) |
...
実装中の判断で Phase B を価値順に B-1〜B-5 に細分化した (一度に巨大な refactor をやらず、価値の高い B-1 から段階的にクローズ可能にするため)。
...
| **C** (v2) | FTS5 trigram opt-in、incognito toggle、retention UI、cache クリアボタン、history view 表示時の thread prefetch (= 全 thread のローカル全 messages 化) | 設定画面・カラムヘッダーに UI 追加 | 未着手 |
...
> 「他のチャットルームは最後のメッセージしか見れない」のは Misskey `chat/history` API が各 thread の最新 1 件しか返さない仕様による (= ユーザーが開いた thread だけ全 messages がキャッシュされる構造)。全 thread のローカル全 messages 化は Phase C で thread prefetch ジョブとして検討。

### 関連設定

```ts
// src/settings/schema.ts に追加予定
'chat.cacheEnabled'?: boolean          // default true
...
| チャットキャッシュ | A-12 `chat_messages_cache`（設計のみ・実装保留 [#460](https://github.com/hitalin/notedeck/issues/460)） |
