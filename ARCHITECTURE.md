...
- [第2部: 残課題](#第2部-残課題)
- [第3部: レンダリングパフォーマンス](#第3部-レンダリングパフォーマンス)
- [採用状況マトリクス](#採用状況マトリクス)
...
TLカラムのノート表示は3層のキャッシュ・キュー階層で管理する。

```mermaid
graph TB
    UI["notes.value<br/>(orderedIds + noteStore)"]
    SQ["StreamingQueue<br/>pendingNotes (auto-flush)<br/>queuedNotes (banner-tap only)"]
    SS["SnapshotStore<br/>in-memory · columnId:cacheKey<br/>TTL eviction · consume/peek"]
...
カラム再マウントやタブ切り替え時の即時復元用 in-memory キャッシュ。
...
- **値**: `{ notes: NormalizedNote[], scrollTop: number, savedAt: number }`
- **eviction**: TTL ベース（`performanceStore.snapshotTTL`）。save 時に expired を一括削除
- **上限**: `performanceStore.snapshotMaxNotes` で保存件数を制限

| 操作 | 用途 | 消費 |
|------|------|------|
| `save(colId, cacheKey, notes, scrollTop)` | スナップショット保存 | — |
...
1. **スナップショットは降順**: notes は `createdAt` 降順で保存。復元時に再ソートしない
2. **noteIds 整合性**: `setNotes()` 後、`noteIds` は `notes.value` の ID と完全一致（`useNoteList` の setter が保証）
3. **StreamingQueue 分離**: `pendingNotes` は auto-flush、`queuedNotes` は明示 flush。混同しない
4. **SnapshotStore TTL**: 唯一の eviction 手段。LRU/容量制限は不要（カラム数はデッキレイアウトで制約）
5. **SQLite は読み取り専用**: フロントエンドは read + delete のみ
...
| レンダリングパフォーマンス | 第3部 → [DEVELOPMENT.md](DEVELOPMENT.md#レンダリングパフォーマンス) に詳細 |
