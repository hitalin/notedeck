...
> **Status**: Phase A + B-1 + B-2 + B-3 + B-5 実装済 (#460)。残りは B-4 (sinceId gap reconcile) と B-6 (thread prefetch)。
...
| **B-6** | history view 表示時の thread prefetch (= 全 thread のローカル全 messages 化) | 「他のチャットルームは最後のメッセージしか見れない」が解消。UI 変更なし | 未着手 |
