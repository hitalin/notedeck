...
```mermaid
graph TD
    Main["メインプロセス"] --> Sandbox["サンドボックス化 WebView<br/>(プラグインごと)"]
    Sandbox --> IPC["制限された IPC のみ許可"]
    Sandbox --> NoAccess["ネットワーク・ファイルアクセス不可"]
    Sandbox --> Whitelist["API はホワイトリスト方式で公開"]
```
...
| OS統合 | A-7 トレイ/ショートカット | C-5 URI スキーム | D-2 ディープリンク, D-4 ウィンドウ透過, D-5 クリップボード |
