...
`src/adapters/types.ts` の `ServerSoftware = 'misskey' | 'unknown'` と `registry.ts` の登録は misskey のみ。`unknown` も misskey にフォールバックする。

**現状の評価**: 「Misskey か否か」の軸で抽象化しているが、[STRATEGY.md](STRATEGY.md) の方針どおり対象は「Misskey を名乗り続けるフォーク」のみ。実際に必要なのは **Misskey 系フォーク間の機能差分**（yamisskey, misskey-tepura 等が持つ独自エンドポイントやレスポンス形式の違い）を吸収する軸。

**改善方針**: adapter パターン自体は残す。ただし `ServerSoftware` の分類を「Misskey vs 非 Misskey」ではなく、**Misskey 系フォーク単位**（misskey-plain / yamisskey / misskey-tepura 等）に再定義し、各 adapter が capability フラグ（`hasAntenna`, `hasClip` 等）で機能差分を宣言する形に組み替える。registry/factory の仕組みはフォーク間の切り替えに活かせる。
...
`composables/useNoteColumn.ts` は fetch / cache / streaming / offline fallback / sound / navigation の 6+ 責務を 1 composable に統合し、14 個のプロパティを返却。
...
| 順序 | 施策 | 効果 | リスク |
|------|------|------|--------|
| 1 | 起動処理を bootstrap orchestrator に一本化 | 起動順序の明示化、機能追加の安全性向上 | 低 |
| 2 | invoke を domain service 経由に集約 | IPC 契約の変更耐性向上、テスト容易性 | 中（段階移行可能） |
| 3 | adapter を capability ベースに組み替え | フォーク対応の実態に合致、抽象化コスト削減 | 低 |
| 4 | streaming と plugin の境界強化 | 再接続整合性、プラグイン安全性 | 中 |
