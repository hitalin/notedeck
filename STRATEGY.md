...
### パフォーマンス — ネイティブだから出せる数字

Tauri v2 + Rust バックエンドにより、Electron / ブラウザベースのクライアントとは桁違いのリソース効率を実現する。
これは単にフレームワークの恩恵ではなく、アプリケーション層で専用の最適化を積んでいる。

| 指標 | Misskey Web UI | Aria (Electron) | NoteDeck |
|---|---|---|---|
| メモリ使用量 | 〜300 MB+ | 〜200 MB | **〜40 MB** |
| 起動時間 | ブラウザ依存 | 数秒 | **< 1 秒** |
