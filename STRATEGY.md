...
NoteDeck より前から Misskey サードパーティクライアントに取り組んでいる **Aria** に敬意を表する。
Aria は Flutter ベースで iOS / Android / デスクトップをカバーする先駆的なプロジェクトであり、
「Misskey にもサードパーティクライアントの選択肢を」という道を切り拓いてきた存在。

NoteDeck は Aria と**競合するつもりはなく、棲み分けたい**と考えている。
アプローチが根本的に異なるため、互いの得意領域で Misskey エコシステム全体を豊かにできるはず。

- **Aria**: Flutter による全プラットフォーム対応。モバイルファーストで幅広いユーザーにリーチ
- **NoteDeck**: Tauri（Vue 3 + Rust）によるデスクトップ特化。OS ネイティブ機能を深く活用

NoteDeck が注力するのは、デスクトップネイティブだからこそ実現できる領域。
デッキ UI、ローカル DB + 全文検索、システムトレイ常駐、localhost API による外部ツール連携、
OS キーチェーン、マルチウィンドウなど — Web やモバイルアプリでは原理的に難しい機能に集中する。
