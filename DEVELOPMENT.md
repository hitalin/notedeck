...
`<Transition>` / `<TransitionGroup>` は Vapor Mode 初期リリースで未サポート（対応時期未定）。
NoteScroller（タイムライン描画の中核）では仮想スクロール導入時に `<TransitionGroup>` → CSS `@keyframes` + データレイヤーマーキングに移行済みで、Vapor 即座対応可能。残りの `<Transition>`（~20箇所）・`<TransitionGroup>`（3箇所）はUI装飾用途で、同パターンで段階的に置換可能。
