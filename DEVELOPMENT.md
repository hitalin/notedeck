...
AI プロバイダー (Anthropic / OpenAI / Custom) の API キーは Misskey トークンと**同じ機構**で管理します — `notecli::keychain` 経由の OS キーチェーン (macOS Keychain / Windows Credential Manager / Linux Secret Service / Android Keystore)。フロントエンドはキー本体に**触れません**。
...
| AI: Anthropic | `notedeck` | `ai.anthropic` |
...
`provider` は `"anthropic" \| "openai" \| "custom"` のいずれか。それ以外は `InvalidInput` エラー。
...
### Secret Vault ([#564](https://github.com/hitalin/notedeck/issues/564)) — 設計確定 (v5.1) / 実装中

AiScript / AI / プラグインが**任意の外部サービス** (GitHub / Linear / 任意の Web API) に接続するための汎用シークレット基盤。Misskey トークン・AI API キーで確立済みの credential proxy モデル (Rust 側で注入、JS / AI には raw secret を渡さない) を任意の外部 API へ拡張する。仕様は 5 round の adversarial review で確定 (170+ 件の指摘を反映)。実装は Phase A〜E に分割。

#### データモデル

| レイヤー | 場所 | 内容 |
|---------|------|------|
| metadata | `<configDir>/notedeck/connections.json5` | 接続定義 (id / name / baseUrl / authType / allowedHosts / accountScope / aiVisible 等)。atomic write + file lock + multi-level unknownFields preserve |
| secret | OS キーチェーン | `service` = `notedeck` (Cargo metadata 可変)、`account` = `vault/v1/<conn_id>/<slot>`。slot で OAuth (v2) 拡張余地を確保 |
| rate limit | `<stateDir>/notedeck/vault/rate.bin` | sliding window、AI/user/HEARTBEAT 独立バケット、atomic write |

`authType` は判別共用体: `{ kind: 'bearer' }` / `{ kind: 'header', name }` / `{ kind: 'query', param }` / `{ kind: 'basic', username }`。

#### Tauri コマンド (`src-tauri/src/commands/vault.rs`)

15 コマンド (`vault_list_connections` / `vault_upsert_connection_with_secret` / `vault_fetch` / `vault_test_connection` / `vault_export` 等)。全コマンド入口で `assert_main_window_token` + `validate_slot` + `validate_conn_id`。`vault_fetch` のみ AI tool から呼べる (他は UI のみ)。HTTP API (19820) からは vault を完全排除、CLI は Tauri IPC + peer credential 検査。

#### `vault.fetch` のセキュリティ

- **SSRF**: HTTP/1.1 only、redirect 各 hop で URL/IP/allowedHosts 再検証、DNS resolver は fetch スコープ共有 (rebinding 防御)、IP deny rules (private/loopback/link-local/multicast)、`normalize_host()` SSoT
- **redaction**: secret は literal memmem で `<vault-redacted-<nonce>>` に置換、drop_headers (Set-Cookie 等)、NFC は text/json のみ、最小 secret 長 16
- **型安全**: secret は `secrecy::SecretString` で workspace 一貫、`Debug` derive 禁止 lint、`panic::set_hook` で backtrace redaction、error は UI 用 / log 用の 2 系統

#### AI 統合

- `aiVisible` toggle (default OFF) で AI 開示を接続単位制御。AI に渡る `<available-connections>` は session 開始時に snapshot 固定
- error は `denied` / `rate_limited` / `external` の 3 値に正規化 (enumeration 防止)、`denied` は 300ms 以上の等価 latency
- AI 起点 fetch は latency 100ms 量子化、rate limit は AI/user/HEARTBEAT 独立バケット
- confirmation scope: `once` / `session-connection` (session-all なし)、同種リクエストは confirm batching

#### permission

`vault.use` (fetch) と `vault.manage` (CRUD) の 2 段。preset: readonly/safe = 両方 false、full = `vault.use` のみ true。HEARTBEAT は両方 false。resolve は deny precedence (個別 deny > 個別 grant > 集合 grant > default deny)。

詳細な設計根拠・脅威モデル・v2 以降の拡張計画は [Issue #564](https://github.com/hitalin/notedeck/issues/564) を参照。
