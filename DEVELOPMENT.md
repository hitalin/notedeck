...
| **IDE ツール** | カラム | 開発・デバッグ支援（Stream Inspector、Workspace Explorer 等） | プロファイルに永続化 |
...
**永続化:**
- 設定はファイル（`ai.json`）+ `settings.json`（`ai.*` キー）+ localStorage の三層保存（dual-write 統合済み）
- **API キーはファイルに含めない**（localStorage のみ保存、セキュリティ対策）
- ファイルはバックアップ・エクスポート対象
- `ai.json` の内容は `settings.json` の `ai.*` キーに dual-write で統合済み。旧ファイルは backward-compat として read-only サポートを継続。API キーの除外方針は維持
...
**永続化:**
- 設定はファイル（`performance.json`）+ `settings.json`（`performance.*` キー）+ localStorage の三層保存（dual-write 統合済み）
- デフォルト値と同じキーはオーバーライドに含めない（差分のみ保存）
- ファイルはバックアップ・エクスポート対象
- バックエンド（Rust）側のパラメータは `invoke('update_performance_config')` で即時同期
- `performance.json` の内容は `settings.json` の `performance.*` キーに dual-write で統合済み。旧ファイルは backward-compat として read-only サポートを継続。差分保存方針は維持
