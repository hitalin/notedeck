...
- **将来**: `ai.json` の内容は `notedeck.json` の `ai.*` キーに統合予定 ([DESIGN.md](DESIGN.md) の「マイグレーション / Phase 3」参照)。API キーの除外方針は維持
...
- **将来**: `performance.json` の内容は `notedeck.json` の `performance.*` キーに統合予定 ([DESIGN.md](DESIGN.md) の「マイグレーション / Phase 3」参照)。差分保存方針は維持
