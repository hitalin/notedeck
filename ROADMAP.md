...
| `commands.rs` | `#[tauri::command]` IPC ハンドラ（notecli を呼び出し） |
| `streaming.rs` | TauriEmitter アダプタ（notecli の FrontendEmitter trait 実装） |
| `http_server.rs` | Axum HTTP API サーバー（localhost:19820） |
| `image_cache.rs` | 3 層イメージキャッシュ（メモリ→ディスク→ネットワーク） |
| `ogp/` | OGP メタデータ抽出＆キャッシュ |
| `query_bridge.rs` | HTTP API ↔ フロントエンド（Pinia）ブリッジ |
...
- ~~**ウィンドウ透過・背景ぼかし**~~ — 廃止。Windows でタイトルバー上の空きスペースが発生する原因となるため削除。
  透過が必要な場合は Misskey カスタムテーマの rgba カラー値で対応
...
- [ ] **連携サンプルコード** — curl / Python / Node.js でのアクセス例。
  Raycast / Obsidian 等の連携は README で紹介し、コミュニティの作成を促す
