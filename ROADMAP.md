...
ノートは 30 日間保持（`cleanup_cache()`）。全カラム種別（home/local/antenna/channel/clip 等）を
...
**実装済みの基礎:**
SQLite FTS5（trigram）でローカル全文検索が動作中。ノートは全カラム種別で自動キャッシュ（30 日保持）。
...
- [ ] **ノートキャッシュ永久保存** — 現状 30 日保持 → 無期限に拡張。
  サーバーからノートが消えてもローカルに残る
- [ ] **TL 履歴タイムマシン** — 日付指定で過去の TL を再現。
  「あの日のタイムラインを見返す」が可能に
...
- [ ] **未読位置の厳密管理** — カラムごとに最終既読位置を永続化。
  アプリ再起動後も正確に「ここまで読んだ」を復元（Phase 2 と連動）
