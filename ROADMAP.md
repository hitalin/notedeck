...
NoteDeck は Misskey エコシステム内のデスクトップ体験（デッキ UI・キーボード操作・ローカル検索）に集中し、
PWA の機能網羅性とネイティブアプリの体験品質を両立する。
...
- [ ] Misskeyについてカラム（Misskey ソフトウェアの紹介・特徴・クレジットを /about-misskey 準拠で表示。meta API からバージョン取得、ソフトウェア説明・特徴は本家準拠の静的コンテンツ）
- [ ] カスタム絵文字カラム（サーバーのカスタム絵文字一覧。カテゴリ別グルーピング・検索フィルタ。既存の絵文字キャッシュを流用）
...
- [ ] 広告カラム（サーバー広告の一覧表示。既存の useAds.ts + meta API の ads フィールドを流用）

---

## Phase 1.5: フォーク adapter — Misskey を名乗るフォークへの対応

**対象範囲: Misskey を名乗り続けるフォークのみ。**
yamisskey・misskey-tempura のような、Misskey の名前・API 体系を維持しつつ独自拡張を加えるフォークに対応する。
Sharkey・CherryPick・Firefish・Iceshrimp 等の名前が別物になるレベルの派生は対象外。

**理由:**
- Misskey ユーザーの大半は日本人で、大型フォーク対応の優先度は高くない
- サポート対象ソフトウェアが乖離しすぎると負担が増大し、サポートが破綻する
- Misskey を名乗るフォークは API 差異が小さく、対応コストが低い
...
## Phase 6: AI 統合 — デスクトップネイティブだから安全に使える
...
- **APIキーの安全な管理**: OSキーチェーン（Keychain / DPAPI / Secret Service）で暗号化保管。
  Web アプリの localStorage とは異なり、XSS・ブラウザ拡張・開発者ツールからの漏洩リスクがない
- **常駐 + ローカル DB + AI の三位一体**: スマホにはローカル DB の蓄積も常駐も弱い
- **ローカル LLM も選択可能**: GPU 環境があれば Ollama でオフライン・無料で動作
- **モバイル版では AI 関連の UI（ナビバーボタン・カラム追加・設定）を非表示にする**

### APIキーのセキュリティ — なぜデスクトップアプリなら安全か
...
Misskey トークンと同じ `keychain.rs` で管理。既存のセキュリティ基盤（OSキーチェーン優先 → SQLite フォールバック → メモリキャッシュ + zeroize）をそのまま活用する。
...
### 6-1. AI プロバイダー統合（段階的に実装）

OpenAI 互換 API を共通インターフェースとし、クラウド API もローカル LLM も同じコードパスで扱う。
翻訳程度のタスクなら API 従量課金は月額数十円レベルなので、実用上のハードルは低い。

```
AI プロバイダー設定（1 画面で完結）:
├── OpenAI 互換エンドポイント（URL + API キー）
│   ├── OpenAI (api.openai.com)        ← GPT-4o mini が安価
│   ├── Anthropic                       ← Haiku が安価
│   ├── OpenRouter (openrouter.ai)      ← 1 キーで複数モデル利用可能
│   ├── Ollama (localhost:11434)        ← 自動検出、GPU 環境向け
│   └── LM Studio (localhost:1234)      ← 自動検出
└── API キー保存先: OS キーチェーン（既存の keychain.rs）
```
...
### 7-1. PiP モード（デスクトップ専用・実装済み）

フルマルチウィンドウより先に、即効性の高い PiP から始める。

- [x] **PiP（ピクチャ・イン・ピクチャ）モード** — 別ウィンドウでコンパクトな TL 監視。
  常に最前面表示（Always on Top）、カラムヘッダーと統一されたデザイン、
  アカウント切替オーバーレイ、ノートクリックでメインウィンドウをフォーカス、
  タイトルバーからトグル（開く/閉じる）操作
...
- [x] **API ドキュメント公開** — OpenAPI 形式で API 仕様を公開（`/api/openapi.json`）。
  Scalar API Reference をアプリ内カラムとして表示。ダーク/ライトテーマ自動切替、ワイドカラム（最大 1200px）対応
...
```
Phase 1.5 (フォーク adapter)    ← Misskey 名乗るフォークのみ。必要に応じて対応
       │
Phase 3 (Hackable)     Phase 5 (廃人モード)     Phase 6 (AI 統合)
  AiScript 基盤           ローカル DB              OpenAI互換API / LLM
       │                      │                      │
       ├──────────┬────────────┤                      │
       │          │            │                      │
  Phase 8      Phase 9        │                      │
...
| 優先度 | やること | 理由 |
|--------|----------|------|
| **今すぐ** | ノート翻訳（AI プロバイダー設定 + OpenAI 互換 API） | Phase 6 の最小統合。ユーザーに見える新しい価値 |
| **次** | ローカルタグ、通知グルーピング、バージョン情報画面 | ユーザー体験の質的向上・寄付導線の整備 |
| **後で** | Nd:* API 拡充、プラグインテンプレート、連携サンプルコード | エコシステム基盤の整備 |
...
| 項目 | 状態 | メモ |
|---|---|---|
| AiScript エディタ | **実装済み** | CodeMirror 6 統合（VS Code Dark+ テーマ・Web Worker LSP 補完/診断・行番号・横スクロール）。ウィジェットカラム + スクラッチパッドカラム（上下分割・リサイズ可能・Ui:render プレビュー・コンソール出力・UI インスペクター）で利用可能 |
...
| 多言語対応（i18n） | 優先度低 | 現状は日本語のみ。Misskey ユーザーの大半は日本人であり、i18n の投資対効果は低い。海外ユーザーからの需要が明確になった段階で検討 |
