...
- [ ] **横断検索** — 複数サーバー × 全カラムのキャッシュを統合検索（Phase 2 連動）。
  単一アカウントの検索カラムでは不可能な、全アカウント横断の検索体験
- [ ] **コマンドパレット** — Ctrl+K の視覚的エントリーポイント。
  全操作の起点であり、特定カラムに属さないアプリ全体の機能
- [ ] **カラム追加** — カラム追加メニューへのショートカット（現在は画面下部のみ）
- [ ] **接続状態インジケーター** — サーバーごとのストリーミング接続状態
  （接続中 / 再接続中 / 切断）をアイコンで表示
- [ ] **設定** — アプリ設定画面へのアクセス
  （現在はアカウントドロップダウン内の Web UI 外部リンクのみ）
- [ ] **テーマ切替** — ダーク / ライト手動切替トグル（現在はシステム連動のみ）
