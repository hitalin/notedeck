...
- [x] **未読管理 / マーカー** — 常駐アプリならではの「どこまで読んだか」追跡。
  カラムごとに `lastReadNoteId` を localStorage 永続化、未読ライン表示、スクロールトップで自動既読

**基盤の現状**: SQLite FTS5 + キャッシュファースト表示 + 差分取得 + 未読管理は実装済み。
...
- [ ] **未読移動** — `.` で次の未読ノートへジャンプ（Phase 2 の未読管理と連動）
...
- [x] **未読位置の厳密管理** — カラムごとに `lastReadNoteId` を localStorage 永続化。
  アプリ再起動後も未読ラインで正確に復元、リスト外の場合は末尾にフォールバック表示
