...
- [x] **NoteDeck 拡張 API（Nd:*）** — Misskey 本家にない NoteDeck 固有の拡張ポイント。
  ...
  **70+ 個の builtin capability** を AiScript からも即時利用可能にしている
...
- [x] **`Command` interface の `Capability` 進化** — `signature` (型シグネチャ) と
  ...
  **全て同じ registry を共有**する。AiScript からは `Nd:call(id, params)` で
  builtin 70+ 個 (notes / column / account / memos / theme / skills / widgets /
...
| **v1.0.0 以降** | PKM 連携 / AI 統合の残 (per-account scope / 細粒度 allow-deny / 自然言語検索 / 自然言語→AiScript / Ollama) / プラグイン開発支援 / i18n / サーバー移転支援 | L3-L4 | 基盤安定後に着手 |
