...
- [ ] **アカウント別予約投稿** — サーバー側の予約投稿 API に委譲。
  NoteDeck 側にキューを持たず、対応サーバーのみ feature detection で有効化（#341）
- [ ] **下書き一覧ウィンドウ** — 複数下書きを並べて比較・整理できる一覧 UI。
  クリックで投稿フォームに復元、個別/全削除。既存の localStorage 下書きを再利用（#342）
...
- [ ] **アカウント別カスタムテーマ** — アカウントごとにカスタムテーマを指定。
  Misskey サーバーの `i/registry/*`（本家 Web UI と同じ scope）にテーマを保存し、
  NoteDeck 側に独自データを持たず Web UI と同期（#339）
...
- [ ] **アカウント別プラグイン管理** — プラグイン一覧・有効状態・`Plugin:config` を
  サーバー側 `i/registry/*`（本家 Misskey Web UI と同じ scope）に保存。
  アカウント単位でプラグインセットを分離しつつ Web UI との互換を維持（#340）
...
| **v1.0.0** | L0〜L2 の残タスク完了 + L3 検索の安定化 | L0-L3 | v1.0.0 リリース条件の達成 |
| **v1.0.0 以降** | PKM 連携、AI 統合、Nd:* API 拡充、プラグイン開発支援、i18n、サーバー移転支援 | L3-L4 | 基盤安定後に着手 |
