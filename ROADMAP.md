...
| ページ | コンテンツ作成ツール。実装コストに見合わない | ✅ アカウントメニュー → Pages |
...
## Phase 6: 常駐アプリの特権 — サーバーに依存しないローカル機能

デスクトップに常駐しているからこそ実現できる、サーバー API に依存しない機能群。
フォークの対応状況に左右されず、どの Misskey 互換サーバーでも動作する。

### 6-1. 時限削除（ローカルスケジューラー）

Misskey 本家には存在しない時限削除を、ローカルスケジューラーで実現する。
サーバーには普通のノートとして投稿されるため、**相手からは削除タイミングが見えない**。

- [ ] **基本時限削除** — 投稿時に「5分後」「30分後」「1時間後」「カスタム」を選択
- [ ] **条件付き削除** — 「リアクション N 件超えたら削除」「特定ユーザーにリプされたら削除」等
- [ ] **削除ログ** — ローカル DB にノートが残るため、消した後も自分だけ見返せる
- [ ] **スケジュール投稿** — 未来の日時を指定して投稿予約。常駐アプリだからスケジューラーが信頼できる
- [ ] **下書き管理** — ローカル永続化、複数下書き、下書きからの予約投稿

### 6-2. ワークスペース（デッキレイアウトの保存・切替）

カラム配置のプリセットを複数保存し、用途に応じて瞬時に切り替える。
デッキ UI のコア価値を倍増させる機能。

- [ ] **名前付きワークスペース** — 「仕事用」「趣味用」「監視用」等、カラム構成をプリセット保存
- [ ] **ワンクリック切替** — ナビバー / コマンドパレットからワークスペースを即切替
- [ ] **時間帯連動** — 朝は通知重視、夜は趣味 TL 重視のような自動切替（オプション）

### 6-3. プライベートアノテーション

サーバーに送信しない、ローカル専用のメタデータ。自分だけの知識ベースを構築する。

- [ ] **ノートメモ** — 他人のノートにローカル専用の付箋・タグを付ける
- [ ] **ユーザーメモ** — 「この人は○○の専門家」等の個人的メモ
- [ ] **ローカルタグ** — クリップとは別の、自分専用の分類体系

### 6-4. パーソナルアナリティクス

SQLite に全ノートが永久保存されているからこそ可能な、サーバーには存在しない統計。

- [ ] **投稿アクティビティ** — 時間帯別・曜日別ヒートマップ
- [ ] **リアクション統計** — もらった/送ったリアクションの推移・傾向
- [ ] **インタラクション分析** — よくやり取りする相手のランキング（サーバー横断）
- [ ] **TL トレンド** — ハッシュタグ・キーワードの出現頻度グラフ

---

## Phase 7: AI Agent Host — Misskey 版 VSCode + Copilot
...
─────────────────────────────────────────────
GitHub Copilot / Claude Code     → Phase 7 で埋める
```

### 7-1. AI パネル（Copilot Chat 相当）
...
- [ ] **AI × 時限削除** — 「炎上しそうだったら自動で消して」等の条件付き自動削除
