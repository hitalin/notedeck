...
- [ ] **Vim キーバインド** — `j` / `k` でノート移動、`gg` / `G` で先頭/末尾、
  `o` で投稿、`r` でリアクション、`Enter` で詳細展開
...
- [ ] **グローバルホットキー投稿** — OS のどこからでも `Ctrl+Shift+N` 等で投稿ウィンドウを呼び出し。
  Twitter 廃人が TweetDeck で愛した機能
...
- [ ] **ノートキャッシュ永久保存** — SQLite にノートを無期限保存。
  サーバーからノートが消えてもローカルに残る
- [ ] **オフライン TL 閲覧** — ネットワーク切断時もキャッシュ済み TL を閲覧可能
- [ ] **TL 履歴タイムマシン** — 日付指定で過去の TL を再現。
  「あの日のタイムラインを見返す」が可能に
- [ ] **全文検索**（Phase 2 横断検索の強化版）— SQLite FTS5 でローカル全ノートを高速検索。
  サーバー API の検索制限に縛られない
