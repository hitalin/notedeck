...
// ── Visual tab: category-based GUI (same as KeybindsContent) ──
...
// Sync visual → code when switching tab
watch(tab, (t) => {
  if (t === 'code') {
...
// Sync store changes → code editor
watch(
...
<template>
  <DeckColumn
    :column-id="column.id"
    :title="column.name ?? 'キーバインド'"
    :theme-vars="columnThemeVars"
  >
    <template #header-icon>
      <i class="ti ti-keyboard" :class="$style.headerIcon" />
    </template>
