...
└── notedeck/               # 設定ファイル群
    ├── notedeck.json           # スカラー preferences（VSCode settings.json 相当）
    ├── profiles/
    │   └── *.ndprofile.json5   # デッキプロファイル（NoteDeck 独自コレクション）
    ├── themes/
    │   └── *.ndtheme.json5     # カスタムテーマ（Misskey 互換フォーマット）
    ├── plugins/
    │   ├── *.is                # AiScript プラグインコード（Misskey 互換）
    │   └── *.meta.json5        # プラグインメタデータ
    ├── custom.css              # カスタムCSS（生の CSS ファイル）
    └── account-order.json5     # アカウント表示順（環境依存、ポータビリティ対象外）
```

**移行中のファイル**（将来 `notedeck.json` に統合予定）:

- `ai.json` → `ai.*` キーに吸収
- `keybinds.json5` → `keybinds.*` キーに吸収
- `performance.json` → `performance.*` キーに吸収

これらはいずれも NoteDeck 独自フォーマットで Misskey 互換性要件がないため、フラット dot-notation キーとして `notedeck.json` に取り込める。バックアップ対象から漏れないようにするための統合。
...
| 形式 | 用途 | 理由 |
|------|------|------|
| JSON | `notedeck.json` (スカラー preferences) | VSCode `settings.json` 相当。フラット dot-notation、`$schema` で JSON Schema 補完 (将来)、外部エディタで編集しやすい |
...
**notedeck.json は JSON (JSON5 ではない)** — VSCode `settings.json` と同じ扱いにするため。将来 JSONC (コメント許可) サポートを検討する余地はあるが、当面は plain JSON。TOML/YAML は現状の設定構造では導入する積極的理由がないため不採用。

### notedeck.json — VSCode settings.json 相当の統合設定ファイル

NoteDeck のスカラー設定 (選択・トグル・ユーザー preferences) は単一ファイル `notedeck.json` に集約する。VSCode の `settings.json` と同じ立ち位置:
...
| パフォーマンス | `performance.maxFps` 等 | 現状 `performance.json`、将来統合 |
| AI | `ai.provider`, `ai.model` 等 | 現状 `ai.json`、将来統合 (API キーは除外) |
| キーバインド | `keybinds.commandPalette` 等 | 現状 `keybinds.json5`、将来統合 |

**notedeck.json に含まれないもの** (恒久的に別ファイル):
...
| `account-order.json5` | 環境依存順序。ポータビリティ対象外 |
...
| **設定** | notedeck.json + プロファイル・テーマ・CSS・プラグイン (+ 移行中: キーバインド・AI設定) | テキストファイル群 |

### バックアップ対象の網羅原則

**原則**: 「バックアップされるもの = 設定」。localStorage のみにしかない設定が存在してはならない。

過去には `modes.realtime` / `theme.manual` / `deck.wallpaper` 等の一部設定が localStorage のみに存在しバックアップから漏れていたが、`notedeck.json` 統合によりこの漏れを解消する。環境依存データ (アカウントトークン / `account-order.json5`) のみが明示的に除外される (他環境での復元時に整合性を壊すため)。
...
| **設定エクスポート** | `notedeck.json` + テキストファイル群 | JSON | `notedeck.json` (スカラー preferences) + `profiles/` `themes/` `plugins/` `custom.css` をキー→値の JSON バンドルとして出力 |
...
**移行中**: 現在の `export_settings_json` は `ai.json` / `keybinds.json5` / `performance.json` を個別ファイルとしてバンドルしているが、これらを `notedeck.json` に統合後、バンドルからは削除される (統合先が `notedeck.json` に含まれるため自動的にカバーされる)。
...
**Phase 2: localStorage-only scalar → notedeck.json** (段階実装中)

Phase 1 でファイル化されなかった localStorage-only 設定 (`theme.manual`, `modes.realtime`, `modes.offline`, `deck.wallpaper` 等) を `notedeck.json` に集約する。

1. 起動時に `notedeck.json` が存在しなければデフォルト値で開始
2. ユーザーが設定を変更するたびに `notedeck.json` に debounce persist
3. 各ストアは `useSettingsStore` 経由で値を取得 (ストア内 localStorage への直接依存を段階的に削除)

**Phase 3: 個別 JSON ファイル → notedeck.json** (計画中)

`ai.json` / `keybinds.json5` / `performance.json` の内容を `notedeck.json` の namespace 付きキーに吸収する。これらは NoteDeck 独自フォーマットで Misskey 互換性要件がないため統合可能:

- `ai.json` → `ai.*` キー
- `keybinds.json5` → `keybinds.*` キー
- `performance.json` → `performance.*` キー

旧ファイルは一定期間 backward-compat として read-only でサポートし、次回 write 時に `notedeck.json` に移行。
