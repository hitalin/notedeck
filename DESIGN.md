...
└── notedeck/               # 設定ファイル群
    ├── settings.json           # スカラー preferences（VSCode settings.json 相当）
    ├── profiles/
...
**移行中のファイル**（将来 `settings.json` に統合予定）:

- `ai.json` → `ai.*` キーに吸収
- `keybinds.json5` → `keybinds.*` キーに吸収
- `performance.json` → `performance.*` キーに吸収

これらはいずれも NoteDeck 独自フォーマットで Misskey 互換性要件がないため、フラット dot-notation キーとして `settings.json` に取り込める。バックアップ対象から漏れないようにするための統合。
...
| **設定** | settings.json + プロファイル・テーマ・CSS・プラグイン (+ 移行中: キーバインド・AI設定) | テキストファイル群 |

### バックアップ対象の網羅原則

**原則**: 「バックアップされるもの = 設定」。localStorage のみにしかない設定が存在してはならない。

過去には `modes.realtime` / `theme.manual` / `deck.wallpaper` 等の一部設定が localStorage のみに存在しバックアップから漏れていたが、`settings.json` 統合によりこの漏れを解消する。環境依存データ (アカウントトークン / `account-order.json5`) のみが明示的に除外される (他環境での復元時に整合性を壊すため)。
...
**移行中**: 現在の `export_settings_json` は `ai.json` / `keybinds.json5` / `performance.json` を個別ファイルとしてバンドルしているが、これらを `settings.json` に統合後、バンドルからは削除される (統合先が `settings.json` に含まれるため自動的にカバーされる)。
...
**Phase 2: localStorage-only scalar → settings.json** (段階実装中)

Phase 1 でファイル化されなかった localStorage-only 設定 (`theme.manual`, `modes.realtime`, `modes.offline`, `deck.wallpaper` 等) を `settings.json` に集約する。
...
**Phase 3: 個別 JSON ファイル → settings.json** (計画中)

`ai.json` / `keybinds.json5` / `performance.json` の内容を `settings.json` の namespace 付きキーに吸収する。これらは NoteDeck 独自フォーマットで Misskey 互換性要件がないため統合可能:

- `ai.json` → `ai.*` キー
- `keybinds.json5` → `keybinds.*` キー
- `performance.json` → `performance.*` キー

旧ファイルは一定期間 backward-compat として read-only でサポートし、次回 write 時に `settings.json` に移行。
